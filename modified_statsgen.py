...
class StatsGen:
    ...
    def generate_stats(self, f):

        f = open('passwords.txt', 'r', encoding="latin-1", errors='ignore')
        for password in f:
	...
	#[+] Analyzing 100% (20/20) of passwords
...
#[*] Advanced Masks:
        ...
        for (advancedmask, count) in sorted(iter(self.stats_advancedmasks.items()), key=operator.itemgetter(1),
            ...
            if count*100//self.filter_counter > 0:
              ...
              #  print(count)
                if x  <= 3:
                    print("%d. %s :   %d " %(x,advancedmask,count))
#           if self.output_file:
#               self.output_file.write("%s,%d\n" % (advancedmask, count))

if __name__ == "__main__":
    header = "Top 3 Password Masks\n"
    header += "--------------------"
